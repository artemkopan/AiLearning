...
@Single
internal class FindMessageOperation(
    ...
    suspend fun execute(
    ...
    ): Result<AgentMessage?> = runtime.value.runDb {
        ScopedAgentMessagesTable.selectAll().where {
            (ScopedAgentMessagesTable.userId eq userId.value.value) and
                ...
                (ScopedAgentMessagesTable.id eq messageId.value.value)
        }.singleOrNull()?.let(mapping.value::toMessage)
    }
}
